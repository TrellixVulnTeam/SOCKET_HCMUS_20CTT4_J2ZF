...
class login(tk.Frame):
    def __init__(self, root):
        ...
        # name page
        __labelName = tk.Label(self, text="LOG IN TO SKYCOV", bg = __WRAPCOLOR, font = "roboto 20 normal")
        __labelName.place(x=35, y=102)

        # username input
        __ipInput = EntryWithPlaceholder(__wrapper, "Username", "#7d7f8e")
        __ipInput.place(x=12, y=170, width=264, height=30)
        # password input
        __portInput = EntryWithPlaceholder(__wrapper, "Password", "#7d7f8e", "yes")
        __portInput.place(x=12, y=216, width=264, height=30)

        # login
        __loginButton = tk.Button(__wrapper, text = "Log In", bg=__BUTTONBGCOLOR, fg=__BUTTONFGCOLOR, activebackground=__BUTTONBGCOLOR_AC, activeforeground=__BUTTONFGCOLOR_AC, font=__BUTTONFONT)
        __loginButton.place(x=12, y=262, width=88, height=36)
        ...
        __signupButton.place(x=189, y=262, width=88, height=36)
        # background on entering widget
